...
ModelHarry<-function(training_data, testing_data, formula){
  trainingMLP <- training_data
  trainingSVMH <- training_data
  testingMLP <- testing_data
  testingSVMH <- testing_data
  # neural network
  ...
  print("training")
  set.seed(123)
  nn=neuralnet(formula,data=trainingMLP,
               stepmax = 8000,
               lifesign.step = 500,
               hidden=layers,
               act.fct="logistic",
               ...
               linear.output = FALSE,
               lifesign = "full")
  predictions<-predict(nn, testingMLP, type="response")
  ...
  supportVectorMachine = svm(formula,
                             ...
                             tolerance = 0.001,
                             probability=TRUE)
  svmPredictions<-predict(supportVectorMachine, testingSVMH, type="response")
