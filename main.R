...
#' Generate a plot from a models predictions using a dataset
#'
#' @param probs vector double - probability of being class 1
#' @param testing_data data frame - dataset to evaluate
#' @param name string - the name of the plot
#'
#' @return
#' @export
#'
...
#'
#' @param probs vector double - probability of being class 1 output from the model
#' @param testing_data data frame - the dataset to evaluate against
#' @param threshold double - cutoff (probability for the classifier)
#'
#' @return list of named evaluation measures and predicted class probabilities
#' @export
#'
#' @examples
...
#' Create a formula for column names and given output
#'
#' @param dataset data frame - the dataset
#' @param fieldNameOutput String - the name of the output field, effectively the
#'   field the model is going to try and predict
#' @param fields list - optional list of field names that you want to use to
#'   make up the forumal instead of all the fields
#'
#' @return R formular object
#' @export
#'
#' @examples
modelFormula <- function(dataset, fieldNameOutput, fields = list()) {
  ...
  if (length(fields) == 0) {
    ...
    inputs <-
      paste(names(dataset)[which(names(dataset) != fieldNameOutput)], collapse = "+")
  } else {
    # use the specified list of fields to make the formula first check that all
    # the listed fields are even in the dataset
...
#' Create a classifier and evaluate
#'
#' @param training_data data frame - the data to train the model on
#' @param testing_data data frame - the data to evaluate the model on
#' @param plot_heading 
#'
#' @return
#' @export
#'
#' @examples
Model <- function(training_data,
                  testing_data,
                  plot_heading) {
  # call the formula function
  formular <-
    modelFormula(dataset = training_data, fieldNameOutput = OUTPUT_FIELD)
  ...
  for (i in seq(1, length(predictionNames))) {
    ...
    if (length(probabilities) > 0) {
      ...
      results <-
        evaluate(probs = probabilities,
                 testing_data = testing_data,
                 threshold = threshold)
      results <-
        displayPerformance(probs = probabilities,
                           testing_data = testing_data,
                           name = name)
    }
  }
...
#' To understand the average time between promotions relative to the time spent
#' with the current manager. This ratio could provide insights into the
#' frequency of promotions in relation to the duration of the current managerial
#' relationship. For example, a higher ratio might suggest that employees tend
#' to receive promotions more frequently in comparison to the time they spend
...
#' Used in creating AgeJoined created from the employee Age and the years they
#' have worked at the company. Provides insights on how the age that they joined
#' the company may affect their loyalty and work ethic toward the company. Also
#' may provide an interesting trend on attrition
#'
#' @param colName1 
#' @param colName2 
#' @param dataframe 
...
#' @examples
  ...
  dataset$PerformanceWithCurrentManager[is.na(dataset$PerformanceWithCurrentManager)] <-
    mean(dataset$PerformanceWithCurrentManager, na.rm = TRUE)
  
  
  #determine each field type
  field_types <-
    getFieldTypes(dataset,
                  continuousFields = CONTINUOUS_FIELDS,
  ...
  continuous <-
    as.data.frame(dataset[which(field_types == TYPE_CONTINUOUS)])
  print("removing outliers")
  continuousWithoutOutliers <-
    removeOutliers(continuous = continuous, confidence = OUTLIER_CONF)
  
  # z-scale
  zscaledWithoutOutliers <-
    as.data.frame(scale(
      continuousWithoutOutliers,
      center = TRUE,
      scale = TRUE
  ...
  continuousWithoutOutliersReadyforML <-
    rescaleDataFrame(zscaledWithoutOutliers)
  ...
  categoricalReadyforML <-
    oneHotEncode(dataset = dataset, field_types = field_types)
  
  
  # Combine the two sets of data that are read for ML
  combinedML <-
    cbind(continuousWithoutOutliersReadyforML,
          categoricalReadyforML)
  
  # process the ordered categorical fields
  print("encoding ordered categorical data")
  orderedCategoricalReadyforML <-
    encodeOrderedCategorical(dataset = dataset, field_types = field_types)
  ...
  # Calling models
  Model(
    training_data = splitList$train,
    testing_data = splitList$test,
    plot_heading = "first dataset outliers removed"
  )
  ...
  Model(
    training_data = splitList$train,
    testing_data = splitList$test,
    plot_heading = "second dataset outliers kept"
  )
